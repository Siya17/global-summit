...
type RuntimeEnv={DB?:D1Database;INSTRUCTOR_PASSWORD?:string}; const runtime=env as unknown as RuntimeEnv;
const db=()=>{if(!runtime.DB)throw new Error("The summit database is not available.");return runtime.DB};
...
export const instructorAuthorized=(request:Request)=>{const expected=runtime.INSTRUCTOR_PASSWORD||process.env.INSTRUCTOR_PASSWORD;return Boolean(expected&&request.headers.get("x-instructor-password")===expected)};
...
const mapRegulation=(x:RegulationRow):Regulation=>({id:x.id,number:x.number,title:x.title,text:x.legal_text,evidenceFor:x.evidence_for,evidenceChallenge:x.evidence_challenge,evidenceDataPoint:x.evidence_data_point,sources:JSON.parse(x.sources_json)});
