...
const mapRegulation=(x:RegulationRow):Regulation=>({id:x.id,number:x.number,title:x.title,text:x.legal_text,provisions:defaultRegulations.find(item=>item.id===x.id)?.provisions,evidenceFor:x.evidence_for,evidenceChallenge:x.evidence_challenge,evidenceDataPoint:x.evidence_data_point,evidenceBars:defaultRegulations.find(item=>item.id===x.id)?.evidenceBars,sources:JSON.parse(x.sources_json)});
